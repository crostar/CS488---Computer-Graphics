...
void A0::guiLogic()
{
	...
	ImGui::Begin("Shape Properties", &showDebugWindow, ImVec2(100,100), opacity,
		...
		// Retrieve color components from slider and store in m_shape_color.
		...
		// Retrieve rotation angle from slider and store in m_shape_rotation
...
//----------------------------------------------------------------------------------------
/*
 * Reset triangle states (colour, scale, rotation, position)
 */
void A0::reset()
{
	m_shape_color = glm::vec3(1.0f, 1.0f, 1.0f);
...
bool A0::keyInputEvent(int key, int action, int mods) {
	...
	if (action == GLFW_PRESS) {
		const float shapeSizeStep = 0.5f;

		if (key == GLFW_KEY_EQUAL) {
			// increase shape size.
			m_shape_size += shapeSizeStep;
			eventHandled = true;
		}
