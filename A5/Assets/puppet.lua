...
green = gr.material({0.4, 0.8, 0.4}, {0.1, 0.1, 0.1}, 50.0)
...
function Sun()
  sun = gr.node('sun')
  sun_instance = gr.mesh('sphere', 'sun_instance')
  sun_instance:set_material(gold)

  sun:add_child(sun_instance)
...
function Earth()
  earth = gr.node('earth')
  earth_instance = gr.mesh('sphere', 'earth_instance')
  earth_instance:scale(0.5, 0.5, 0.5)
  earth_instance:set_material(green)

  earth:add_child(earth_instance)
  earth:translate(5.0, 0.0, 0.0)
  return earth
end
